...
const SECTION_LABELS = {
  ...
  educationResources: ["Education Resources", "教育资源"],
  medicalPharmacyResources: ["Medical & Pharmacy", "医疗与药房"],
    ...
    <div className="admin-page strategy-reports-page">
      ...
      {loading ? <p>{isZh ? "正在读取报告…" : "Loading reports…"}</p> : (
        <div className="card strategy-reports-table-wrap"><table className="strategy-reports-table"><thead><tr>{["Assessment ID", "Created At", "Owner Name", "Email", "Phone", "Property Address", "City", "Community", "Property Type", "Target Rent", "Language", "Status", "View Report", "Open PDF"].map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>
          {filtered.map((item) => { const pdfUrl = item.reportUrls?.[isZh ? "zh" : "en"] || item.reportUrl; return <tr key={item.assessmentId}><td>{item.assessmentId}</td><td>{item.createdAt || "-"}</td><td>{item.ownerName || "-"}</td><td>{item.email || "-"}</td><td>{item.phone || "-"}</td><td>{item.propertyAddress || "-"}</td><td>{item.city || "-"}</td><td>{item.community || "-"}</td><td>{item.propertyType || "-"}</td><td>{item.targetRent || "-"}</td><td>{item.language}</td><td>{item.status}</td><td><button className="btn btn--secondary btn--small" onClick={() => viewReport(item.assessmentId)}>{isZh ? "查看报告" : "View Report"}</button></td><td>{pdfUrl ? <a className="btn btn--secondary btn--small" href={pdfUrl} target="_blank" rel="noreferrer">{isZh ? "管理员内部 Drive PDF" : "Admin Internal Drive PDF"}</a> : <span className="text-muted">{isZh ? "PDF 尚未生成" : "PDF Not Generated"}</span>}</td></tr>; })}
      ...
      {selected && <div className="strategy-report-modal" role="dialog" aria-modal="true" aria-label="Strategy assessment report"><div className="strategy-report-modal__panel"><div className="strategy-report-modal__header"><div><h2>{selected.assessmentId}</h2><p>{selected.language} · {selected.status}</p></div><button className="btn btn--secondary" onClick={() => setSelected(null)}>{isZh ? "关闭" : "Close"}</button></div>
        <div className="strategy-report-detail-meta">{["Owner Name", "Email", "Phone", "Property Address", "City", "Community ID", "Community Name", "Community / Area", ...(selected.fields?.["Property Building Type"] || selected.fields?.["Rental Unit Type"] ? ["Property Building Type", "Rental Unit Type"] : ["Property Type"]), "Outdoor Space Type", "Fence Status", "Laundry Type", "Utilities Arrangement", "Shared Areas", "Target Rent", "Follow-up Answers", "AI Flags"].map((key) => selected.fields?.[key] ? <div key={key}><strong>{key}</strong><span>{selected.fields[key]}</span></div> : null)}</div>
        {(selected.reportUrls?.[isZh ? "zh" : "en"] || selected.reportUrl) ? <p><a className="btn btn--primary" href={selected.reportUrls?.[isZh ? "zh" : "en"] || selected.reportUrl} target="_blank" rel="noreferrer">{isZh ? "管理员内部 Drive PDF" : "Admin Internal Drive PDF"}</a></p> : <div className="notice notice--warning"><p>{isZh ? "PDF 尚未生成" : "PDF Not Generated"}</p></div>}
        <div className="strategy-report-detail-sections">{(selected.reports?.[isZh ? "zh" : "en"] || selected.analysis) ? Object.entries(selected.reports?.[isZh ? "zh" : "en"] || selected.analysis).map(([key, value]) => <section key={key}><h3>{SECTION_LABELS[key]?.[isZh ? 1 : 0] || key}</h3><ReportValue value={value} /></section>) : <section><h3>AI Analysis</h3><p className="strategy-report-raw">{selected.analysisText || "No saved report content."}</p></section>}</div>
      </div></div>}
    </div>
  );
}
