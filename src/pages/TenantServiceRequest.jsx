...
const zh = {
  title: "租客报修与服务申请", verify: "验证租客身份", email: "电子邮箱", phone: "电话号码后四位", verifyButton: "验证", verifying: "正在验证…", securityCompleted: "安全验证已完成", identityVerified: "租客身份验证成功", generic: "我们无法验证你的资料，请联系物业经理。",
...
export default function TenantServiceRequest({ lang = "en" }) {
  ...
  return <main className="tsr-page"><section className="tsr-card">
    ...
    {step === "verify" && <form onSubmit={verify}><h2>{t.verify}</h2><label>{t.email}<input name="email" type="email" autoComplete="email" required /></label><label>{t.phone}<input name="phoneLast4" inputMode="numeric" pattern="[0-9]{4}" maxLength="4" required /></label><div className={`tsr-turnstile-shell${turnstileToken ? " tsr-turnstile-shell--complete" : ""}`}><div ref={widgetRef} className="tsr-turnstile" />{turnstileToken && <p className="tsr-security-complete" role="status">✓ {t.securityCompleted}</p>}</div><button className="tsr-primary-action" type="submit" disabled={busy || !turnstileToken} aria-label={busy ? t.verifying : t.verifyButton}><span>{busy ? t.verifying : t.verifyButton}</span></button></form>}
