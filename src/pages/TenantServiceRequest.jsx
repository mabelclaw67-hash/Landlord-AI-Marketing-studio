...
export default function TenantServiceRequest({ lang = "en" }) {
  ...
  return <main className="tsr-page"><section className="tsr-card">
    ...
    {step === "verify" && <form onSubmit={verify}><h2>{t.verify}</h2><label>{t.email}<input name="email" type="email" autoComplete="email" required /></label><label>{t.phone}<input name="phoneLast4" inputMode="numeric" pattern="[0-9]{4}" maxLength="4" required /></label><div className={`tsr-turnstile-shell${turnstileToken ? " tsr-turnstile-shell--complete" : ""}`}><div ref={widgetRef} className="tsr-turnstile" />{turnstileToken && <p className="tsr-security-complete" role="status" style={{ padding: 0, border: 0, borderRadius: 0, background: "transparent", color: "#607068", fontSize: ".82rem", fontWeight: 400, lineHeight: 1.45 }}>{t.securityCompleted}</p>}</div><button className="tsr-primary-action" type="submit" disabled={busy || !turnstileToken} aria-label={busy ? t.verifying : t.verifyButton}><span>{busy ? t.verifying : t.verifyButton}</span></button></form>}
    {step === "confirm" && <div><p className="tsr-identity-verified" role="status">✓ {t.identityVerified}</p><h2>{t.property}</h2><p className="tsr-property">{session.propertyLabel}</p><label className="tsr-check"><input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} /> {t.confirm}</label><button disabled={!confirmed} onClick={() => setStep("form")}>{t.details}</button></div>}
